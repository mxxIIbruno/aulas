...
CAMINHO_ZIP_DIR = CAMINHO_RAIZ / 'aula_30_diretorio_zip'
CAMINHO_COMPACTADO = CAMINHO_RAIZ / 'aula30_compactado.zip'
CAMINHO_DESCOMPACTADO = CAMINHO_RAIZ / 'aula30_descompactado'
...
def criar_arquivos(qtd: int, zip_dir: Path):
    for i in range(qtd):
        text = 'arquivo_%s' % i
        with open(zip_dir / f'{text}', 'w', encoding='uft8') as file:
            file.write(text)
